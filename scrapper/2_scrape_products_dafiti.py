...
def pick_images(soup, jsonld_products):
    urls = []
    # 1) JSON-LD "image"
    for p in jsonld_products:
        imgs = p.get("image")
        if isinstance(imgs, str):
            ...
            for u in imgs:
                if isinstance(u, str):
                    urls.append(u)
    # 2) og:image (podem vir várias metas)
    for m in soup.find_all("meta", {"property": "og:image"}):
        u = m.get("content")
        if u:
            urls.append(u)
    # 3) Galeria HTML (somente imagens de produto)
    for img in soup.find_all("img", src=True):
        src = img["src"]
        if "dafitistatic" in src and "-product" in src and not any(t in src for t in ["logo", "icon", "placeholder", "thumb"]):
            urls.append(src)

    # normaliza, remove duplicatas, garante https
    norm = []
    seen = set()
    for u in urls:
        if not u:
